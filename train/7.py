...
if not os.path.isdir('./Checkpoint/7/'):
    os.mkdir('./Checkpoint/7/')
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            torch.save(model, './Checkpoint/7/model-epoch-{:d}-train.pth'.format(epoch + 1))
    ...
    best_model_name = './Checkpoint/7/model-{:.2f}-best_train_acc.pth'.format(best_acc)
