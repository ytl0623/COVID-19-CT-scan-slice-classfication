...
if not os.path.isdir('./Checkpoint/16/'):
    os.mkdir('./Checkpoint/16/')
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            torch.save(model, './Checkpoint/16/model-epoch-{:d}-train.pth'.format(epoch + 1))
    ...
    best_model_name = './Checkpoint/16/model-{:.2f}-best_train_acc.pth'.format(best_acc)
