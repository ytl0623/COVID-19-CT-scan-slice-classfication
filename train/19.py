...
if not os.path.isdir('./Checkpoint/19/'):
    os.mkdir('./Checkpoint/19/')
...
def train():
    ...
    train_dl = DataLoader(trainset, batch_size=16, shuffle=True, num_workers=3)
    validset = CovidCT("./input/", "valid", data_transform)
    valid_dl = DataLoader(validset, batch_size=16, shuffle=False, num_workers=3)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            torch.save(model, './Checkpoint/19/model-epoch-{:d}-train.pth'.format(epoch + 1))
    ...
    best_model_name = './Checkpoint/19/model-{:.2f}-best_train_acc.pth'.format(best_acc)
