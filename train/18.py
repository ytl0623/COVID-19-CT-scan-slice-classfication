...
if not os.path.isdir('./Checkpoint/18/'):
    os.mkdir('./Checkpoint/18/')
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            torch.save(model, './Checkpoint/18/model-epoch-{:d}-train.pth'.format(epoch + 1))
    ...
    best_model_name = './Checkpoint/18/model-{:.2f}-best_train_acc.pth'.format(best_acc)
