...
if not os.path.isdir('./Checkpoint/6/'):
    os.mkdir('./Checkpoint/6/')
...
def train():
    ...
    model.fc=nn.Linear(in_features=2048, out_features=2, bias=True) #如果要使用預訓練模型，記得修改最後一層輸出的class數量
    #print(model)
    print("==========")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0:
            torch.save(model, './Checkpoint/6/model-epoch-{:d}-train.pth'.format(epoch + 1))
    ...
    best_model_name = './Checkpoint/6/model-{:.2f}-best_train_acc.pth'.format(best_acc)
